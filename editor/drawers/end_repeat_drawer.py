...
class EndRepeatDrawerMixin:
    def draw_end_repeat(self, du: DrawUtil) -> None:
        ...
        if layout is None or not bool(getattr(layout, 'repeat_end_visible', True)):
            return

        events = list(getattr(score.events, 'end_repeat', []) or [])
        if not events:
            return
        ...
        for ev in events:
            ...
            # Vertical timeline orientation: end-repeat is a thicker barline extension
            # on the right side, with two side-by-side dots above it.
            du.add_line(
                x_left,
                y,
                x_right,
