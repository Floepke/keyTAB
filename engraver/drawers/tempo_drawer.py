...
def tempo_drawer(du, pre_calc: dict) -> None:
    ...
    for stv in list(line.get('staves', []) or []):
        ...
        for ev in list(events.get('tempo', []) or []):
            txt = str((ev.get('text', None) if isinstance(ev, dict) else None) or 'q=120')
            x = float(stv.get('note_span_left_x_mm', 0.0) or 0.0)
            du.add_text(x, y0 - 1.0, txt, family='Edwin', size_pt=9.0, color=notation_color, anchor='w', tags=['tempo_text'])
